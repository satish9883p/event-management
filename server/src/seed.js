...
async function seed() {
  try {
    ...
    // 1. Create Users (plain text — pre-save hook hashes)
    const adminUser = await User.create({
      name: 'Event Pro Admin',
      email: 'admin@eventpro.com',
      password: 'admin123',
      role: 'admin',
      phone: '+91 98765 43210',
      organization: 'Event Pro Headquarters',
    });
    ...
    // 2. Seed all 36 States/UTs with districts and areas
    let locationDocs = [];
    ...
    console.log(`Inserted ${locationDocs.length} location nodes across 36 States/UTs of India.`);

    // 3. Seed Function Halls
    const hallsWithAdmin = HALLS_DATA.map(h => ({ ...h, createdBy: adminUser._id }));
    const hallDocs = await FunctionHall.insertMany(hallsWithAdmin);
    console.log(`Inserted ${hallDocs.length} Function Halls across India.`);

    // 4. Seed Events
    const eventsWithAdmin = EVENTS_DATA.map(e => ({
      ...e,
      createdBy: adminUser._id,
    }));
    const eventDocs = await Event.insertMany(eventsWithAdmin);
    console.log(`Inserted ${eventDocs.length} Pan-India Events.`);

    // 5. Seed 2 sample confirmed bookings (for demonstration)
    const sampleHall = hallDocs[0];
    const sampleEvent = eventDocs[0];

    const booking1Id = `EP-20260903-DEMO`;
    const booking2Id = `EP-20260903-EVT1`;
    ...
    await Booking.insertMany([
      {
        ...
        basePrice: sampleHall.price,
        additionalServices: [{ name: 'Floral Stage & Arch Decoration', price: 15000 }],
        totalPrice: sampleHall.price + 15000,
        paymentStatus: 'paid',
        bookingStatus: 'confirmed',
        razorpayOrderId: 'order_DEMO_00001',
        razorpayPaymentId: 'pay_DEMO_00001',
        paymentMethod: 'upi',
        guestCount: 450,
        contactName: demoUser.name,
        contactPhone: demoUser.phone,
        contactEmail: demoUser.email,
        specialRequests: 'Require flower entrance by 3 PM',
      },
      {
        ...
        timeSlot: `${sampleEvent.startTime} – ${sampleEvent.endTime}`,
        basePrice: sampleEvent.price,
        additionalServices: [],
        totalPrice: sampleEvent.price,
        paymentStatus: 'paid',
        bookingStatus: 'confirmed',
        razorpayOrderId: 'order_DEMO_00002',
        razorpayPaymentId: 'pay_DEMO_00002',
        paymentMethod: 'card',
        guestCount: 1,
        contactName: demoUser.name,
        contactPhone: demoUser.phone,
    ...
    console.log('Inserted 2 sample confirmed Bookings.');

    console.log('\n✅ Seeding completed successfully!');
    console.log('──────────────────────────────────────────────');
    console.log('Admin Login : admin@eventpro.com / admin123');
    console.log('User Login  : user@eventpro.com / user123');
    console.log('──────────────────────────────────────────────');
    console.log(`States/UTs covered : ${INDIA_LOCATIONS.length}`);
    console.log(`Location nodes     : ${locationDocs.length}`);
    console.log(`Function Halls     : ${hallDocs.length}`);
